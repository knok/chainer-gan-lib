...
# Color 128
class C128Generator(chainer.Chain):
    def __init__(self, n_hidden=128, bottom_width=4, ch=512, wscale=0.02,
                 z_distribution="uniform", hidden_activation=F.relu, output_activation=F.tanh, use_bn=True):
        super(C128Generator, self).__init__()
        self.n_hidden = n_hidden
        self.ch = ch
        self.bottom_width = bottom_width
        self.z_distribution = z_distribution
        ...
        with self.init_scope():
            ...
            self.dc6 = L.Deconvolution2D(ch // 32, 3, 3, 1, 1, initialW=w)
            if self.use_bn:
                self.bn0 = L.BatchNormalization(bottom_width * bottom_width * ch)
                self.bn1 = L.BatchNormalization(ch // 2)
                self.bn2 = L.BatchNormalization(ch // 4)
    ...
    def __call__(self, z):
        if not self.use_bn:
            ...
            h = self.hidden_activation(self.dc2(h))
            h = self.hidden_activation(self.dc3(h))
            h = self.hidden_activation(self.dc4(h))
            h = self.hidden_activation(self.dc5(h))
...
class SND128Discriminator(chainer.Chain):
    def __init__(self, bottom_width=4, ch=512, wscale=0.02, output_dim=1):
        w = chainer.initializers.Normal(wscale)
        super(SND128Discriminator, self).__init__()
        with self.init_scope():
            self.c0_0 = SNConvolution2D(3, ch // 32, 3, 1, 1, initialW=w)
            self.c0_1 = SNConvolution2D(ch // 32, ch // 16, 4, 2, 1, initialW=w)
            self.c1_0 = SNConvolution2D(ch // 16, ch // 16, 3, 1, 1, initialW=w)
            self.c1_1 = SNConvolution2D(ch // 16, ch // 8, 4, 2, 1, initialW=w)
            self.c2_0 = SNConvolution2D(ch // 8, ch // 8, 3, 1, 1, initialW=w)
            self.c2_1 = SNConvolution2D(ch // 8, ch // 4, 4, 2, 1, initialW=w)
            self.c3_0 = SNConvolution2D(ch // 4, ch // 4, 3, 1, 1, initialW=w)
            self.c3_1 = SNConvolution2D(ch // 4, ch // 2, 4, 2, 1, initialW=w)
            self.c4_0 = SNConvolution2D(ch // 2, ch // 2, 3, 1, 1, initialW=w)
            self.c4_1 = SNConvolution2D(ch // 2, ch // 1, 4, 2, 1, initialW=w)
            self.c5_0 = SNConvolution2D(ch // 1, ch // 1, 3, 1, 1, initialW=w)
            self.l6 = SNLinear(bottom_width * bottom_width * ch, output_dim, initialW=w)
    ...
    def __call__(self, x):
        ...
        h = F.leaky_relu(self.c2_1(h))
        h = F.leaky_relu(self.c3_0(h))
        h = F.leaky_relu(self.c3_1(h))
        h = F.leaky_relu(self.c4_0(h))
        h = F.leaky_relu(self.c4_1(h))
        h = F.leaky_relu(self.c5_0(h))
        return self.l6(h)
